...
- 文章解析链接：[『论文笔记』Convolutional Block Attention Module(注意力机制)+TensorFlow2.0复现！](https://blog.csdn.net/abc13526222160/article/details/103765484)

### 一. 论文摘要

本文提出了卷积块注意模块(CBAM)，这是一个简单而有效的前馈卷积神经网络注意模块。在给定中间特征图的情况下，我们的模块按照通道和空间两个独立的维度依次推断注意图，然后将注意图乘入输入特征图进行自适应特征细化。因为CBAM是一个轻量级的通用模块，它可以无缝地集成到任何CNN架构中，开销可以忽略不计，并且可以与基本CNNs一起进行端到端的培训。我们通过在ImageNet-1K、MS COCO检测和VOC 2007检测数据集上的大量实验来验证我们的CBAM。我们的实验表明，在不同的模型下，分类和检测性能都得到了一致的提高，说明了CBAM的广泛适用性。

### 二. CBAM的网络结构
#### 2.1. 总体的描述

对于一个中间层的<img src="https://latex.codecogs.com/gif.latex?$\mathbf&space;F&space;\in&space;\mathbb{R}^{C&space;\times&space;H&space;\times&space;W}$" title="$\mathbf F \in \mathbb{R}^{C \times H \times W}$" />，CBAM将会顺序推理出1维的channel attention map <img src="https://latex.codecogs.com/gif.latex?$\mathbf&space;M_{c}&space;\in&space;\mathbb{R}^{C&space;\times&space;1&space;\times&space;1}$" title="$\mathbf M_{c} \in \mathbb{R}^{C \times 1 \times 1}$" /> 以及2维的spatial attention map <img src="https://latex.codecogs.com/gif.latex?$\mathbf&space;M_{s}&space;\in&space;\mathbb{R}^{1&space;\times&space;H&space;\times&space;W}$" title="$\mathbf M_{s} \in \mathbb{R}^{1 \times H \times W}$" />，整个过程如下所示： 
...
**主要思路就是：增大有效通道权重，减少无效通道的权重。** 公式表示为如下：
...
**其中：** <img src="https://latex.codecogs.com/gif.latex?$\mathbf{F}_{\text&space;{avg}}^\mathbf{c}$" title="$\mathbf{F}_{\text {avg}}^\mathbf{c}$" /> 和 <img src="https://latex.codecogs.com/gif.latex?$\mathbf{F}_{\text&space;{max}}^\mathbf{c}$" title="$\mathbf{F}_{\text {max}}^\mathbf{c}$" /> 表示对feature map在空间维度上使用**最大池化**和**平均池化**。<img src="https://latex.codecogs.com/gif.latex?$\mathbf{W}_{0}&space;\in&space;\mathbb{R}^{C&space;/&space;r&space;\times&space;C},&space;\quad&space;\mathbf{W}_{1}&space;\in&space;\mathbb{R}^{C&space;\times&space;C&space;/&space;r}$" title="$\mathbf{W}_{0} \in \mathbb{R}^{C / r \times C}, \quad \mathbf{W}_{1} \in \mathbb{R}^{C \times C / r}$" />，<img src="https://latex.codecogs.com/gif.latex?$\mathbf{W}_{0}$" title="$\mathbf{W}_{0}$" /> 后使用了Relu作为激活函数，<img src="https://latex.codecogs.com/gif.latex?$\sigma$" title="$\sigma$" /> 表示**Sigmoid**函数。
...
<img src="https://latex.codecogs.com/gif.latex?$$\begin{aligned}\mathbf{M}_{\mathbf{s}}(\mathbf{F})&space;&=\sigma\left(f^{7&space;\times&space;7}(\left[\text&space;{AvgPool}(\mathbf{F})&space;;&space;\text&space;{MaxPool}(\mathbf{F})]\right))\right)&space;\\&space;&=\sigma\left(f^{7&space;\times&space;7}\left(\left[\mathbf{F}_{\text&space;{avg&space;}}^{\mathrm{s}}&space;;&space;\mathbf{F}_{\text&space;{max&space;}}^{\mathrm{s}}\right]\right)\right)\tag{4}&space;\end{aligned}&space;$$" title="$$\begin{aligned}\mathbf{M}_{\mathbf{s}}(\mathbf{F}) &=\sigma\left(f^{7 \times 7}(\left[\text {AvgPool}(\mathbf{F}) ; \text {MaxPool}(\mathbf{F})]\right))\right) \\ &=\sigma\left(f^{7 \times 7}\left(\left[\mathbf{F}_{\text {avg }}^{\mathrm{s}} ; \mathbf{F}_{\text {max }}^{\mathrm{s}}\right]\right)\right)\tag{4} \end{aligned} $$" />
